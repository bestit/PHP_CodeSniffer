...
class TagSortingSniff extends AbstractSniff
{
    ...
    private function checkAndRegisterLineBreakErrors(): void
    {
        ...
        foreach ($tokens as $tokenPos => $token) {
            ...
            if ($isGroupSwitch && (($prevToken['line'] + 1) === $token['line'])) {
                $isFixing = $this->getFile()->addFixableWarning(
    ...
    private function removeOldTagLines(): array
    {
        ...
        (new LineHelper($this->getFile()))
            ->removeLines(
                $firstTag['line'],
                $this->tokens[$this->getToken()['comment_closer']]['line']
            );
    ...
    {
        $this->docTagHelper = new DocTagHelper($this->getToken(), $this->getFile(), $this->getStackPosition());
    }
